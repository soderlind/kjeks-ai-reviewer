...
- Support single-site installs: the `suggest` / `accept` REST routes fall back to `manage_options` when not multisite (previously required `manage_network`, which no user holds on a single-site install).
